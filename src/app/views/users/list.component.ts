...
export class ListComponent implements OnInit {
    ...
    public sortFields = [{"Id": "id", "Name":"Id"}, {"Id": "Name", "Name":"Name"}, {"Id": "ShortCode", "Name":"Short code"}, {"Id": "Published", "Name":"Published"}, {"Id": "Ordering", "Name":"Order"}, { "Id": "CreatedAt", "Name": "Date Created"},{ "Id": "UpdatedAt", "Name": "Date Updated"}];
    ...
    itemsPerPageOptions = [5, 10, 20, 25, 50, 100, 250, 500];
  	...
  	ngOnInit() {
		this._titleService.setTitle(this._t('Users.List'));
        this.globals = myGlobals;
        this.acl = this._auth.createACL('users');
    ...
    getItems() {
        this.loading = true;
        let apiUrl=   'users?limit=' + this.itemsPerPage + '&page=' + this.currentPage + '&sort=' + this.sortField + '|' + this.sortOrder;  // URL to web API
    ...
    deleteItem(index: string) {
        if (this._dialogLib.showDialog(this._t('DialogMsgs.Delete?') + ': ' + this.items[index].Title + '?') != true) return false;
        this.loading = true;
        let apiUrl = 'users/' + this.items[index].uid;
        this._netService.authDelete(apiUrl)
            .subscribe(
                res => {
                    this.successMsg = '<h2>' + this._t('DialogMsgs.Deleted') + '</h2>' + this._t('DialogMsgs.DeleteSuccess') + ': ' + this.items[index].Title;
    ...
    updateToggleField(fIndex: string, fName: string, fValue: number) {
        this.updateItem(this.items[fIndex]['uid'], fName, fValue);
        this.items[fIndex][fName] = fValue;
    }
